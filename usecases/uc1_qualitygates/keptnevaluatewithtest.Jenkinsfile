...
node {
    properties([
        parameters([
         ...
         string(defaultValue: '/:homepage;/echo:echo;/version:version;/invoke?url=www.keptn.sh:invoke', description: 'A semi-colon separated list of URIPaths:TestName tupples that the load test should generate load', name: 'URLPaths', trim: false),
    ...
    stage('Initialize Keptn') {
        ...
        keptn.keptnInit project:"${params.Project}", service:"${params.Service}", stage:"${params.Stage}", keptnConfigureMonitoring:"${monitoring}" // , shipyard:'shipyard.yaml'
